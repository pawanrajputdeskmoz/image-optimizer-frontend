...
import type { Paginated, WorkerLogRow } from "@/app/admin/_lib/types";
...
export default function LogsPageClient() {
  ...
  const [data, setData] = useState<Paginated<WorkerLogRow> | null>(null);
  ...
  const load = useCallback(async (f = filters, p = page) => {
    setLoading(true);
    setError(null);
    const res = await adminApi.logs({ ...f, page: p, limit: 20 });
    if (res.error) {
      setError(res.error);
      setData(null);
    } else {
      setData(res.data as Paginated<WorkerLogRow>);
    }
    setLoading(false);
  }, [filters, page]);
  ...
  async function openDetail(id: string) {
    const res = await adminApi.log(id);
    if (res.data) setDetail(res.data as WorkerLogRow);
  }
  ...
  return (
    <div>
      <PageHeader
        title="Worker Logs"
        subtitle="Auto-deleted after 4 days"
          ...
          <AdminTable
            ...
            rows={data.items.map((log) => ({
              ...
              cells: [
                ...
                <Btn key="a" small onClick={() => void openDetail(log._id)}>
                  Details
                </Btn>,
              ],
            }))}
